    ...
    def _create_tables(self):
        ...
        try:
            with self.conn.cursor() as cursor:
                # 0. User accounts table (required for Multi-user SaaS)
                cursor.execute("""
                ...
                # 2. Volume anomalies (huge trades or orderbook walls)
                cursor.execute("""
                ...
                # 3. Trading signals generated by analysis
                cursor.execute("""
                ...
                # 4. Executed trades (paper or live)
                cursor.execute("""
                ...
                # 6. Database schema migrations (add user_id to existing tables if needed)
                cursor.execute("ALTER TABLE signals ADD COLUMN IF NOT EXISTS user_id INTEGER REFERENCES users(id)")
                cursor.execute("ALTER TABLE trades ADD COLUMN IF NOT EXISTS user_id INTEGER REFERENCES users(id)")
                cursor.execute("ALTER TABLE portfolio_snapshots ADD COLUMN IF NOT EXISTS user_id INTEGER REFERENCES users(id)")
